...
class WorkerIO(object):
    ...
    def __init__(self, input_spectrum_file, input_feature_file, output_file=None, type="DIA"):
        ...
        print("output_file = {0:s}".format(str(self.output_file)))
    ...
    def split_feature_index(self):
        ...
        index_batch_list = [
            self.feature_index_list[i : (i + self.batch_size)]
            for i in range(0, self.feature_count["total"], self.batch_size)
        ]
...
class WorkerI(object):
    ...
    def get_spectrum(self, feature_index_batch, input_feature_file_handle, input_spectrum_file_handle):
        ...
        for feature_index in feature_index_batch:
            ...
            # update dataset
            # 更新数据集
            # scan_list_middle 居中对称，选择离rt_mean 最近的作为中线
            spectrum = {
                "feature_id": feature_id,  # str(feature_index),#scan,
                "feature_area": feature_area,
                "raw_sequence": raw_sequence,
            ...
            spectrum_list.append(spectrum)
        # 输出一下 spectrum_list的size是多大
        return spectrum_list

    # 读取feature.csv文件中的特征属性，并返回
    ...
    def _parse_spectrum(
        ...
        ### select best neighbors from the scan_list by their distance to rt_mean
        # 选择scans中和rt mean距离最近的一个scan index作为best scan
        # probably move this selection to get_location(), run once rather than repeating
        neighbor_count = len(scan_list)
        best_scan_index = None
        best_distance = float("inf")
