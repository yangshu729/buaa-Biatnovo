  ...
  if not opt.beam_search:
    ...
    if opt.direction == 0 or opt.direction == 2:
      ...
      for index in xrange(decoder_size):
        ...
        # candidate_intensity = np.zeros(shape=(deepnovo_config_dda.vocab_size, deepnovo_config_dda.neighbor_size * deepnovo_config_dda.num_ion, deepnovo_config_dda.WINDOW_SIZE), dtype=np.float32)
        # import pdb; pdb.set_trace()
        candidate_intensity = get_candidate_intensity_dda(
          spectrum_original_forward,# (5, 150000)
          precursor_mass,
          prefix_mass,
          0)
  ...
  if opt.multiprocessor == 1:
    with open(worker_i.input_feature_file, 'r') as feature_fr :
      with open(worker_i.input_spectrum_file, 'r') as spectrum_fr:
        ...
        #               peptide_ids_backward,
        #               ms1_profile]
        result_list = [read_single_spectrum(feature_index, worker_i, feature_fr, spectrum_fr, opt)
