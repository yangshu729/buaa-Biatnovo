...
from DataProcessing.read_dda import read_random_stack
  ...
  for length_idx in xrange(decoder_size):
    ...
    batch_decoder_inputs_forward.append(
      np.array([decoder_inputs_forward[batch_idx][length_idx]
                for batch_idx in xrange(batch_size)], dtype=np.int32))  # (12, batchsize)
...
def train_cycle(model,
  ...
  while True:
    ...
    loss, n_correct_forward, n_correct_backward, n_word = cal_performance_focal_loss(output_logits_forward,
...
def train(opt):
    ...
    feature_index_list_train = list(feature_index_list_train)
    random.shuffle(feature_index_list_train)
    ...
    for epoch in range(start_epoch + 1, deepnovo_config_dda.epoch_stop):
        ...
        print("epoch :", str(epoch), " ", "epoch_loss :", str(epoch_loss / step_len) + " " + "epoch word forward accuarcy:",
              str(epoch_word_correct_forward / (epoch_word_total / 2)), " epoch word backward accuarcy:",
              str(epoch_word_correct_backward / (epoch_word_total / 2)), " epoch word accuarcy:",
              str(epoch_word_correct_total / epoch_word_total))
        epoch_time = time.time() - start_time
        print("epoch_time:", epoch_time)
        valid_word_correct_forward, valid_word_correct_backward, valid_word_correct_total, valid_word_total, valid_loss = valid_test(model, opt, valid_set, valid_bucket_pos_id)
...
def main():
    ...
    parser.add_argument('--train_feature', type=str, default="train_feature",
                        help="Feature csv file to train a new model.")
    parser.add_argument('--valid_spectrum', type=str, default="train_spectrum",
                        help="Spectrum mgf file for validation during training.")
    parser.add_argument('--valid_feature', type=str, default="train_feature",
                        help="Feature csv file for validation during training.")
    ...
    parser.add_argument('--cuda', action='store_true', default=False,
                        help="Set to True to use gpu.")
    parser.add_argument('--lstm_kmer', action='store_true', default=False,
    ...
    parser.add_argument('--multiprocessor', type=int, default=1,
