...
def train_cycle(model, worker_io_train, feature_index_list_train, opt, optimizer, training_mode, step):
    ...
    train_set, _ = read_random_stack(
        worker_io_train, feature_index_list_train, deepnovo_config_dda.train_stack_size, opt, training_mode, step
    )
    train_bucket_sizes = [len(train_set[b]) for b in xrange(len(deepnovo_config_dda._buckets))]
    ...
    while True:
        ...
        if train_current_spectra[bucket_id] + deepnovo_config_dda.batch_size >= train_bucket_sizes[bucket_id]:
            print("train_current_spectra ", train_current_spectra)
            print("train_bucket_sizes ", train_bucket_sizes)
            break
        index_list = range(
            train_current_spectra[bucket_id], train_current_spectra[bucket_id] + deepnovo_config_dda.batch_size
        )
        # get_batch_01/2
        (
            spectrum_holder,
...
def valid_test(model, opt, valid_set, valid_bucket_pos_id):
    ...
    for bucket_id in valid_bucket_pos_id:
        ...
        data_set_index_chunk_list = [
            data_set_index_list[i : i + deepnovo_config_dda.batch_size]
...
def train(opt):
    ...
    worker_io_train = deepnovo_worker_io.WorkerIO(
        input_spectrum_file=opt.train_spectrum, input_feature_file=opt.train_feature, type="DDA"
    )
    worker_io_valid = deepnovo_worker_io.WorkerIO(
        input_spectrum_file=opt.valid_spectrum, input_feature_file=opt.valid_feature, type="DDA"
    )
    ...
    valid_set, valid_set_len = read_random_stack(
        worker_io_valid, feature_index_list_valid, deepnovo_config_dda.valid_stack_size, opt
    )
    ...
    for epoch in range(start_epoch + 1, deepnovo_config_dda.epoch_stop):
        ...
        for step in range(step_len):
            ...
            print(
                ...
                " ",
                "step_loss :",
                str(step_loss / deepnovo_config_dda.train_stack_size) + " " + "step word forward accuarcy:",
        ...
        print(
            ...
            " valid loss:",
            str(valid_loss / deepnovo_config_dda.valid_stack_size),
        )
