...
def test_logit_single_2_score(decoder_input_forward,
...
def test_logit_batch_2_score(decoder_inputs_forward,
                       decoder_inputs_backward,
  ...
  for batch in xrange(batch_size):
    ...
    dec_string, score_sum, position_score = test_logit_single_2_score(decoder_input_forward,
                                                                      decoder_input_backward,
...
def test_accuracy_score(model, data_set, bucket_id, opt):
    ...
    data_set_index_chunk_list = [data_set_index_list[i:i + deepnovo_config.batch_size_predict]
                                 for i in range(0,
                                                data_set_len,
                                                deepnovo_config.batch_size_predict)]
    index = 0
    bi_score = []
    for chunk in data_set_index_chunk_list:
        ...
        output_batch = test_logit_batch_2_score(decoder_inputs_forward[:decoder_size],
...
def test_accuracy_score_dda(model, data_set, bucket_id, opt):
    ...
    data_set_index_list = range(data_set_len)
    data_set_index_chunk_list = [data_set_index_list[i:i + deepnovo_config_dda.batch_size_predict]
                                 for i in range(0,
                                                data_set_len,
                                                deepnovo_config_dda.batch_size_predict)]
    index = 0
    bi_score = []
    for chunk in data_set_index_chunk_list:
        ...
        (spectrum_holder,
         ...
         decoder_inputs_forward,
         decoder_inputs_backward,
         target_weights) = get_batch_2_dda(chunk, data_set, bucket_id)
