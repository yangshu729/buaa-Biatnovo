...
class WorkerDenovo(object):
    ...
    def search_denovo_bi_indepedent(self, model, worker_io, opt, type="DIA"):
        ...
        for index, feature_index_batch in enumerate(worker_io.feature_index_batch_list):
            print("Read {0:d}/{1:d} batches".format(index + 1,
                                                    worker_io.feature_index_batch_count))
            spectrum_batch = worker_io.get_spectrum(feature_index_batch)
            predicted_batch_forward, predicted_batch_backward = self._search_denovo_batch_bi_indepedent(spectrum_batch, model)
            ...
            for spectrum_id in xrange(spectrum_batch_size):
                if len(predicted_batch_forward[spectrum_id]['sequence'][0]) > 0 and len(
                        predicted_batch_backward[spectrum_id]['sequence'][0]) > 0:
                    concate_result = self.concate_more(predicted_batch_forward[spectrum_id]['sequence'][0],
                ...
                elif len(predicted_batch_forward[spectrum_id]['sequence'][0]) > 0 and len(
                        predicted_batch_backward[spectrum_id]['sequence'][0]) == 0:
                    ...
                    continue
                if type == "DIA":
                    result_list = [read_single_spectrum_true_feeding(spectrum_batch[spectrum_id], sequence, opt) for
                                   sequence in concate_result]
                elif type == "DDA":
                    result_list = [
                        read_dda.read_single_spectrum_true_feeding(spectrum_batch[spectrum_id], sequence, opt) for
                        sequence in
                        concate_result]
            ...
            for spectrum_id in xrange(spectrum_batch_size):
                if len(result_score[spectrum_id]) > 0:
                    ...
                    if len(score_sum_list) > 0:
                        ...
                        predicted_batch_forward[spectrum_id]['sequence'] = [result_score[spectrum_id][tmp_i]['dec_string']]
                        predicted_batch_forward[spectrum_id]['score'] = [result_score[spectrum_id][tmp_i]['score_sum']]
                        predicted_batch_forward[spectrum_id]['position_score'] = [result_score[spectrum_id][tmp_i]['position_score']]
    ...
    def search_denovo_bi_SB(self, model, worker_io, opt, type="DIA"):
        ...
        for index, feature_index_batch in enumerate(worker_io.feature_index_batch_list):
            print("Read {0:d}/{1:d} batches".format(index + 1,
                                                    worker_io.feature_index_batch_count))
            spectrum_batch = worker_io.get_spectrum(feature_index_batch)
            predicted_batch_forward, predicted_batch_backward = self._search_denovo_batch_bi_SB(spectrum_batch, model)
            spectrum_batch_size = len(spectrum_batch)
            data_set = [[] for _ in self._buckets]
            spectrum_index = [[] for _ in self._buckets]
            result_score = [[] for x in xrange(spectrum_batch_size)]
    ...
    def search_denovo_bi_SB_indepedent(self, model, model_ind, worker_io, opt):
        ...
        for index, feature_index_batch in enumerate(worker_io.feature_index_batch_list):
            ...
            spectrum_batch = worker_io.get_spectrum(feature_index_batch)
            predicted_batch_forward, predicted_batch_backward = self._search_denovo_batch_bi_SB(spectrum_batch, model)
            predicted_batch_forward_indepedent, predicted_batch_backward_indepedent = self._search_denovo_batch_bi_indepedent(spectrum_batch, model_ind)
            if len(predicted_batch_forward) == 0 and len(predicted_batch_backward) == 0 and len(predicted_batch_forward_indepedent) == 0\
            ...
            if len(predicted_batch_forward[0]["score"]) > 0 and len(predicted_batch_backward[0]["score"]) > 0:
                concate_result = self.concate(predicted_batch_forward[0]['sequence'][0], predicted_batch_backward[0]['sequence'][0],
                                              predicted_batch_forward[0]['precursor_mz'] * predicted_batch_forward[0]['precursor_charge'])
                if len(predicted_batch_forward_indepedent[0]["score"]) > 0 and len(predicted_batch_backward_indepedent[0]["score"]) > 0:
                ...
                if len(predicted_batch_forward_indepedent[0]["score"]) > 0 and len(
                        predicted_batch_backward_indepedent[0]["score"]) > 0:
            ...
            if len(concate_result) > 0:
                if type == "DIA":
                    result_list = [read_single_spectrum_true_feeding(spectrum_batch, sequence, opt) for sequence in concate_result]
                elif type == "DDA":
                    result_list = [read_dda.read_single_spectrum_true_feeding(spectrum_batch, sequence, opt) for sequence in
                                   concate_result]
                data_set = [[] for _ in self._buckets]
        ...
        for index, feature_index_batch in enumerate(worker_io.feature_index_batch_list):
            ...
            for spectrum_id in xrange(spectrum_batch_size):
                if len(predicted_batch_forward) == 0 and len(predicted_batch_backward) == 0 and len(predicted_batch_forward_indepedent) == 0\
                    ...
                    continue
                # 交互式预测只有都输出才进行拼接
                if len(predicted_batch_forward[spectrum_id]["score"]) > 0 and len(predicted_batch_backward[spectrum_id]["score"]) > 0:
                    concate_result = self.concate_more(predicted_batch_forward[spectrum_id]['sequence'][0], predicted_batch_backward[spectrum_id]['sequence'][0],
                                                predicted_batch_forward[spectrum_id]['precursor_mz'] * predicted_batch_forward[spectrum_id]['precursor_charge'])
                    if len(predicted_batch_forward_indepedent[spectrum_id]["score"]) > 0 and len(predicted_batch_backward_indepedent[spectrum_id]["score"]) > 0:
                        # 独立预测只有都输出才进行拼接
                        concate_result_inde = self.concate_more(predicted_batch_forward_indepedent[spectrum_id]['sequence'][0], predicted_batch_backward_indepedent[spectrum_id]['sequence'][0],
                                                                predicted_batch_forward_indepedent[spectrum_id]['precursor_mz'] * predicted_batch_forward_indepedent[spectrum_id]['precursor_charge'])
                    ...
                    if len(predicted_batch_forward_indepedent[spectrum_id]["score"]) > 0 and len(
                            predicted_batch_backward_indepedent[spectrum_id]["score"]) > 0:
                        concate_result = self.concate_more(predicted_batch_forward_indepedent[spectrum_id]['sequence'][0], predicted_batch_backward_indepedent[spectrum_id]['sequence'][0],
                                                        predicted_batch_forward_indepedent[spectrum_id]['precursor_mz'] * predicted_batch_forward_indepedent[spectrum_id]['precursor_charge'])
                    elif len(predicted_batch_forward_indepedent[spectrum_id]["score"]) > 0:
    ...
    def search_denovo_bi_SB_indepedent_all_1(self, model, model_ind, worker_io, opt, type="DIA"):
        ...
        for index, feature_index_batch in enumerate(worker_io.feature_index_batch_list):
            ...
            # 交互式预测只有都输出才进行拼接
            if len(predicted_batch_forward[0]["score"]) > 0 and len(predicted_batch_backward[0]["score"]) > 0:
                concate_result = self.concate_more(predicted_batch_forward[0]['sequence'][0], predicted_batch_backward[0]['sequence'][0],
                                              predicted_batch_forward[0]['precursor_mz'] * predicted_batch_forward[0]['precursor_charge'])
                if len(predicted_batch_forward_indepedent[0]["score"]) > 0 and len(predicted_batch_backward_indepedent[0]["score"]) > 0:
                    # 独立预测只有都输出才进行拼接
                    concate_result_inde = self.concate_more(predicted_batch_forward_indepedent[0]['sequence'][0], predicted_batch_backward_indepedent[0]['sequence'][0],
                                                            predicted_batch_forward_indepedent[0]['precursor_mz'] * predicted_batch_forward_indepedent[0]['precursor_charge'])
                    for i in concate_result_inde:
                        concate_result.append(i)
                elif len(predicted_batch_forward_indepedent[0]["score"]) > 0:
                    concate_result.append(tuple(predicted_batch_forward_indepedent[0]['sequence'][0]))
                ...
                if len(predicted_batch_forward_indepedent[0]["score"]) > 0 and len(
                        predicted_batch_backward_indepedent[0]["score"]) > 0:
                    concate_result = self.concate_more(predicted_batch_forward_indepedent[0]['sequence'][0], predicted_batch_backward_indepedent[0]['sequence'][0],
                                                       predicted_batch_forward_indepedent[0]['precursor_mz'] * predicted_batch_forward_indepedent[0]['precursor_charge'])
                elif len(predicted_batch_forward_indepedent[0]["score"]) > 0:
                    concate_result = predicted_batch_forward_indepedent[0]['sequence']
                elif len(predicted_batch_backward_indepedent[0]["score"]) > 0:
            ...
            if len(concate_result) > 0:
                if type == "DIA":
                    result_list = [read_single_spectrum_true_feeding(spectrum_batch[0], sequence, opt) for sequence in concate_result]
                elif type == "DDA":
                    result_list = [read_dda.read_single_spectrum_true_feeding(spectrum_batch[0], sequence, opt) for sequence in
                                   concate_result]
                data_set = [[] for _ in self._buckets]
                bucket_feature = [[] for _ in self._buckets]
                index = 0
        ...
        while True:
            ...
            for entry_index, search_entry in enumerate(active_search_list):
                ...
                for path in current_path_list:
                    ...
                    candidate_intensity = self.get_candidate_intensity(spectrum_original,
                                                                  precursor_mass,
                                                                  prefix_mass,
                                                                  direction_id)
            ...
            if block_AAid_1:
                ...
                with torch.no_grad():
                    ...
                    for i in range(candidate_size):
                        ...
                        log_prob = model.Inference(spectrum_cnn_outputs, candidate_intensity, decoder_inputs, direction_id)
            ...
            active_search_list = [x for x in active_search_list if
                                  x["current_path_list"]]  
        ...
        while True:
            ...
            if block_AAid_1:
                ...
                if block_AAid_1.size(0) == 1 and block_AAid_1[0][0] == block_AAid_2[0][0]:
                    ...
                    block_decoder_inputs = torch.cat([block_AAid_1, block_AAid_2], dim=0)
                with torch.no_grad():
                    log_prob = model.Inference(block_spectrum_cnn_outputs, block_candidate_intensity, block_decoder_inputs, direction_id)
                    Logsoftmax = torch.nn.LogSoftmax(dim=1)
                    log_prob = Logsoftmax(log_prob)
        ...
        while True:
            ...
            for entry_index, search_entry in enumerate(active_search_list_l2r):
                ...
                for path in current_path_list_l2r:
                    ...
                    if AAid_2 == LAST_LABEL_l2r:  # nobi
                        if (abs(prefix_mass - precursor_mass) <= peak_mass_tolerance):
                            top_path_batch_l2r[spectrum_id].append({"AAid_list": AAid_list,
                                                                    "score_list": score_list,
                                                                    "score_sum": score_sum})
            ...
            for entry_index, search_entry in enumerate(active_search_list_r2l):
                ...
                for path in current_path_list_r2l:
                    ...
                    if AAid_2 == LAST_LABEL_r2l:  # nobi
                        if (abs(prefix_mass - precursor_mass) <= peak_mass_tolerance):
                            top_path_batch_r2l[spectrum_id].append({"AAid_list": AAid_list,
                                                                    "score_list": score_list,
                                                                    "score_sum": score_sum})
            ...
            if not all(flag_l2r) or not all(flag_r2l):
                ...
                with torch.no_grad():
                    ...
                    block_l2r = [[i,j,k,l] for i,j,k,l in zip(block_spectrum_id_l2r, block_spectrum_cnn_outputs_l2r, block_candidate_intensity_l2r, [block_decoder_inputs_l2r[:,f] for f in range(block_decoder_inputs_l2r.size(1))] )]
                    block_r2l = [[i,j,k,l] for i,j,k,l in zip(block_spectrum_id_r2l, block_spectrum_cnn_outputs_r2l, block_candidate_intensity_r2l, [block_decoder_inputs_r2l[:,f] for f in range(block_decoder_inputs_r2l.size(1))] )]
                    ...
                    log_prob_l2r, log_prob_r2l = model.Inference_SB(b_spectrum_cnn_outputs, b_candidate_intensity_l2r, b_candidate_intensity_r2l,
                                                                     b_decoder_inputs_l2r, b_decoder_inputs_r2l)                        
                    Logsoftmax = torch.nn.LogSoftmax(dim=1)
            ...
            for entry_index, search_entry in enumerate(active_search_list_l2r):
                ...
                spectrum_id = search_entry["spectrum_id"]
                new_path_list = []
                for index in xrange(block_index_l2r, block_index_l2r + search_entry_size_l2r[entry_index]):
                    for AAid in block_knapsack_candidate_l2r[index]:
                        ...
                        if AAid > 2:  # do NOT add score of GO, EOS, PAD
                            try:
                                ...
                                new_path["score_sum"] = (block_score_sum_l2r[index]
                                                        + current_log_prob_l2r[index][AAid])
                ...
                if len(new_path_list) > self.beam_size // 2:
                    try:
                        ...
                        search_entry["current_path_list_l2r"] = [new_path_list[top_k_index[x]]
            ...
            active_search_list_l2r = [x for x in active_search_list_l2r if
                                  x["current_path_list_l2r"]]  

            block_index_r2l = 0
            for entry_index, search_entry in enumerate(active_search_list_r2l):
                spectrum_id = search_entry["spectrum_id"]
                current_log_prob_r2l_specturm = [i for i,j in zip(current_log_prob_r2l, b_specturm_id) if j==spectrum_id]
                # find all possible new paths within knapsack filter
                new_path_list = []
                for index in xrange(block_index_r2l, block_index_r2l + search_entry_size_r2l[entry_index]):
                    for AAid in block_knapsack_candidate_r2l[index]:
                        ...
                        if AAid > 2:  # do NOT add score of GO, EOS, PAD
                            ...
                            new_path["score_sum"] = (block_score_sum_r2l[index]
                                                     + current_log_prob_r2l[index][AAid])
                        else:
                ...
                if len(new_path_list) > self.beam_size // 2:
                    try:
                        new_path_score = np.array([x["score_sum"].cpu() for x in new_path_list])
                        top_k_index = np.argsort(-new_path_score)[:self.beam_size // 2] 
                        search_entry["current_path_list_r2l"] = [new_path_list[top_k_index[x]]
                                                                for x in xrange(self.beam_size // 2)]
                    except Exception as inst:
                        new_path_score = np.array([x["score_sum"] for x in new_path_list])
                        top_k_index = np.argsort(-new_path_score)[:self.beam_size // 2] 
            ...
            active_search_list_r2l = [x for x in active_search_list_r2l if
                                      x["current_path_list_r2l"]]  
            ...
            if not all(flag_l2r) or not all(flag_r2l):
                ...
                with torch.no_grad():
                    ...
                    for i in range(min(candidate_size_l2r, candidate_size_r2l)):
                        ...
                        log_prob_l2r, log_prob_r2l = model.Inference_SB(spectrum_cnn_outputs, candidate_intensity_l2r, candidate_intensity_r2l,
                                                                     decoder_inputs_l2r, decoder_inputs_r2l)
                        Logsoftmax = torch.nn.LogSoftmax(dim=1)
                        log_prob_l2r = Logsoftmax(log_prob_l2r)
    ...
    def _search_denovo_batch(self, spectrum_batch, model):
        ...
        for peak_batch in peak_list:
            forward_path_batch = self._extend_peak_batch("forward",
                                                   model,
                                                   spectrum_batch,
                                                   peak_batch)
            backward_path_batch = self._extend_peak_batch("backward",
                                                    model,
                                                    spectrum_batch,
                                                    peak_batch)
            for spectrum_id in xrange(spectrum_batch_size):
    ...
    def _search_knapsack(self, mass, knapsack_tolerance):
        ...
        candidate_AAid = np.flatnonzero(np.any(self.knapsack_matrix[:, mass_lowerbound_col:mass_upperbound_col + 1],
    ...
    def _select_peak(self, spectrum_batch):
        ...
        for spectrum in spectrum_batch:
            ...
            argmax_location = np.argpartition(-spectrum_forward[self.neighbor_center, :precursor_mass_C_location],
                                              self.num_position)[:self.num_position]  # pylint: disable=line-too-long
            ...
            if not candidate_list:  # cannot find any peptide
                ...
                if (len(candidate_list) > self.topk_output):
                    topk_index = np.argpartition(-score_array, self.topk_output)[
                                 :self.topk_output]  # pylint: disable=line-too-long
                    predicted_list = [candidate_list[index] for index in topk_index]
                else:
                    predicted_list = candidate_list
                predicted_batch[spectrum_id]["score"] = [predicted["score"].cpu() / len(predicted["sequence"])
                ...
                predicted_batch[spectrum_id]["sequence"] = [[self.vocab_reverse[x] for x in predicted["sequence"]]
                                                            for predicted in predicted_list]
        return predicted_batch